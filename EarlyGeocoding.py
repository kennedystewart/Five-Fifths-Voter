...
def scrape(county, skip_existing, apikey):
    ...
    if TESTURL:
        ...
        scrapeURL = 'https://voterportal.sos.la.gov/earlyvoting'
    html_text = requests.get(scrapeURL).text
    soup = BeautifulSoup(html_text, 'html.parser')
    table = soup.select('#EarlyVotingLocations')
    
    #TODO: payload["Parish"] should match the params file
    row = table[0].find(text=payload["parish"]).find_next('td')
    ...
    for r in range(len(row.contents)):
        ele = row.contents[r]
        if (ele.name == 'div'):
            address = ' '.join([ele.contents[i].replace("*", "").strip() 
                      for i in range(len(ele))if (ele.contents[i].name != 'br' and 
                      not re.compile("^\d{3}-\d{3}-\d{4}$").match(ele.contents[i].strip()))])
